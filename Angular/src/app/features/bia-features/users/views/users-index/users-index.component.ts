...
import { getChangeCount } from 'src/app/domains/bia-domains/user-option/store/user-option.state';
...
export class UsersIndexComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.sub.add(
      this.store.select(getChangeCount).pipe(skip(1)).subscribe(event => {
