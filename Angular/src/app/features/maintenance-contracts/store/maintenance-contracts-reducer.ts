...
export const maintenanceContractsAdapter = createEntityAdapter<MaintenanceContract>({
  selectId: (maintenanceContract: MaintenanceContract) => maintenanceContract.id,
  sortComparer: false,
});
...
export interface State extends CrudState<MaintenanceContract>, EntityState<MaintenanceContract> {
...
export const maintenanceContractReducers = createReducer<State>(
  ...
  on(FeatureMaintenanceContractsActions.loadAllByPostSuccess, (state, { result, event }) => {
    const stateUpdated = maintenanceContractsAdapter.setAll(result.data, state);
    stateUpdated.totalCount = result.totalCount;
    stateUpdated.lastLazyLoadEvent = event;
    stateUpdated.loadingGetAll = false;
    return stateUpdated;
  }),
  on(FeatureMaintenanceContractsActions.loadSuccess, (state, { maintenanceContract }) => {
