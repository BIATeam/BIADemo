...
import { loadAllSuccess } from './role-options-actions';
...
export const roleOptionReducers = createReducer<State>(
  INIT_STATE,
  on(loadAllSuccess, (state, { roles }) => roleOptionsAdapter.setAll(roles, state)),
  // on(loadSuccess, (state, { role }) => roleOptionsAdapter.upsertOne(role, state))
);
