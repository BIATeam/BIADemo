...
import { HttpOptions, HttpRequestItem } from 'bia-ng/models';
import { HttpStatusCodeCustom } from 'bia-ng/models/enum';
import {
  BehaviorSubject,
  Observable,
  Subscription,
...
export class BiaOnlineOfflineService implements OnDestroy {
    ...
    if (options) {
      if (options.headers) {
        options.headers = (<HttpHeaders>options.headers).append(
          BiaOnlineOfflineService.httpHeaderRetry,
          'true'
        );
      } else {
      ...
      if (
        ...
        this.biaMessageService.showWarning(
          this.translateService.instant(
            'biaMsg.serverUnavailableDataSaveLocally'
          )
        );
      }
  ...
  protected initObsSyncCompleted() {
    this.sub.add(
      this.syncCompleted$
        ...
        .subscribe(() => this.biaMessageService.showSyncSuccess())
    );
  }
  ...
  protected ping(): Observable<string> {
    return this.http.get<string>(
      BiaEnvironmentService.getServerLoggingUrl() + '/ping'
    );
  }
  ...
  protected httpRequestPost(httpRequestItem: HttpRequestItem) {
    const obs$ = this.http.post(
      httpRequestItem.httpRequest.urlWithParams,
      httpRequestItem.httpRequest.body
    );
  ...
  protected async addHttpRequestItem(httpRequest: HttpRequest<any>) {
    await this.db.httpRequests.add(<HttpRequestItem>{
  ...
  protected async deleteHttpRequestItem(httpRequestItem: HttpRequestItem) {
    if (httpRequestItem && httpRequestItem.id) {
      const nextHttpRequestItem: HttpRequestItem | null =
        await this.takeNextHttpRequest(httpRequestItem);
  ...
  protected async launchFirstHttpRequestItem() {
    const nextHttpRequestItem: HttpRequestItem | null =
      await this.takeFirstHttpRequest();
    if (nextHttpRequestItem) {
      await this.sendHttpRequest(nextHttpRequestItem);
    }
  }
  ...
  protected async takeFirstHttpRequest() {
    const httpRequestItems: HttpRequestItem[] =
      await this.getAllHttpRequestItem();
  ...
  protected async takeNextHttpRequest(httpRequestItem: HttpRequestItem) {
    ...
    if (httpRequestItems?.length > 0) {
      const index = httpRequestItems.findIndex(
        x => x.id === httpRequestItem.id
      );
