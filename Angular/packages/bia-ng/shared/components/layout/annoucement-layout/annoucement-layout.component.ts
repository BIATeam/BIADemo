...
import { BiaAnnoucementType } from 'packages/bia-ng/models/enum/bia-annoucement-type.enum';
import { SafeHtmlPipe } from '../../../pipes/safe-html.pipe';

@Component({
  selector: 'bia-annoucement-layout',
  imports: [SafeHtmlPipe, TranslateModule],
  templateUrl: './annoucement-layout.component.html',
  styleUrl: './annoucement-layout.component.scss',
})
export class AnnoucementLayoutComponent
  implements OnChanges, AfterViewInit, OnDestroy
{
  @Input() messages: Annoucement[] | null = null;
      ...
      this.formatedMessages = (this.messages ?? [])
        .map(message => {
          ...
          switch (message.type.id) {
            case BiaAnnoucementType.Info:
              iconElement =
                '<i class="pi pi-info-circle annoucement-icon-info"></i>';
              break;
            case BiaAnnoucementType.Warning:
              iconElement =
                '<i class="pi pi-exclamation-triangle annoucement-icon-warning"></i>';
              break;
          }
          return iconElement + message.rawContent;
        })
        .join('<span class="annoucement-separator">|</span>');
