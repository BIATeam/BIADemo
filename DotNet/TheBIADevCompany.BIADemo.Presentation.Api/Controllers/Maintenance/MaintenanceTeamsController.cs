...
namespace TheBIADevCompany.BIADemo.Presentation.Api.Controllers.Maintenance
{
    ...
    public class MaintenanceTeamsController : BiaControllerBase
    {
...
#endif
        {
            ...
            this.EntityName = "maintenance-team";
            this.EntityNamePlural = "maintenance-teams";
#endif
        ...
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_List_Access))]
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_Read))]
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_Create))]
...
#if UseHubForClientInMaintenanceTeam
                await this.SendEntityChangedAsync(createdDto.AircraftMaintenanceCompanyId.ToString());
#endif
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_Update))]
...
#if UseHubForClientInMaintenanceTeam
                await this.SendEntityChangedAsync(updatedDto.AircraftMaintenanceCompanyId.ToString());
#endif
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_Delete))]
...
#if UseHubForClientInMaintenanceTeam
                await this.SendEntityChangedAsync(deletedDto.AircraftMaintenanceCompanyId.ToString());
#endif
                return this.Ok();
            }
            catch (ElementNotFoundException)
...
#if UseHubForClientInMaintenanceTeam
                await this.SendEntityChangedAsync(parentKeys: deletedDtos.Select(m => m.AircraftMaintenanceCompanyId.ToString()).Distinct().ToList());
#endif
                return this.Ok();
            }
            catch (ElementNotFoundException)
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_Save))]
...
#if UseHubForClientInMaintenanceTeam
                await this.SendEntityChangedAsync(parentKeys: savedDtos.Select(m => m.AircraftMaintenanceCompanyId.ToString()).Distinct().ToList());
#endif
                return this.Ok();
            }
            catch (ArgumentNullException)
        ...
        public virtual async Task<IActionResult> GetFile([FromBody] PagingFilterFormatDto<TeamAdvancedFilterDto> filters)
        {
            ...
            return this.File(buffer, BiaConstants.Csv.ContentType + $";charset={BiaConstants.Csv.CharsetEncoding}", $"MaintenanceTeams{BiaConstants.Csv.Extension}");
        }
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [Authorize(Roles = nameof(PermissionId.MaintenanceTeam_Fix))]
