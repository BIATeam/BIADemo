// <copyright file="IOperationalDomainServiceBase.cs" company="BIA">
...
namespace BIA.Net.Core.Domain.Service
{
    ...
    public interface IOperationalDomainServiceBase<TDto, TDtoListItem, TEntity, TKey, TFilterDto>
        ...
        where TFilterDto : class, IPagingFilterFormatDto, new()
    {
        ...
        Task<(IEnumerable<TDtoListItem> Results, int Total)> GetRangeAsync(
        ...
        /// A <see cref="Task" /> representing the asynchronous operation.
        /// </returns>
        Task<byte[]> GetCsvForImportAsync(
            TFilterDto filters = null,
            TKey id = default,
            Specification<TEntity> specification = null,
            Expression<Func<TEntity, bool>> filter = null,
        ...
        /// <param name="isReadOnlyMode">if set to <c>true</c> [This improves performance and enables parallel querying]. (optionnal, false by default).</param>
        /// <returns>The list of DTO.</returns>
        Task<IEnumerable<TDtoListItem>> GetAllAsync(
            TKey id = default,
            Specification<TEntity> specification = null,
            Expression<Func<TEntity, bool>> filter = null,
