...
namespace BIA.Net.Core.Common.Configuration
{
  ...
  public class Jwt
  {
    ...
    /// <summary>
    /// Gets or sets a value indicating whether to use compact permission IDs only (remove string permissions from claims).
    /// When true, only urn:bia:permission_ids is emitted (smaller JWT).
    /// When false (default), both permission IDs and legacy string permissions are emitted (backward compatibility).
    /// </summary>
    public bool UseCompactPermissionsOnly { get; set; }
  }
}